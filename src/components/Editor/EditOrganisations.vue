            ...
            >
              ...
              >
                ...
                <v-file-input
                  ...
                  disabled
                />
                <div class="orgaPreview" v-if="menus.validity.newOrganisation">
                  ...
                  >
                    ...
                    <v-list-item-content class="mb-2">
                      ...
                      <v-list-item-subtitle v-if="menus.data.organisation_type_ids">
                        Types: {{ menus.data.organisation_type_ids.join(", ") }}
                      </v-list-item-subtitle>
                    </v-list-item-content>
                  </v-list-item>
                </div>
  ...
  export default {
      ...
      methods: {
        async getData(){
          ...
          getOrganisationsQuery.queryParam = {load: false};
          data = await client.executeQuery(getOrganisationsQuery);
        ...
        async createNewOrganisation(){
          ...
          if (logo) {
            ...
            reader.addEventListener('load', async (event) => {
              ...
              await this.saveNewOrganisation(organisationInput);
            });
        ...
        async saveNewOrganisation(organisation){
          organisation.alternativeNames = this.menus.data.alternativeNames ? this.menus.data.alternativeNames.split(",") : null;
          if (!organisation.alternativeNames) delete organisation.alternativeNames;
          let data = await restClient.createOrganisation(organisation, this.user().credentials.token);
          console.log(data);
          if (data.error) {
            ...
            let newOrganisation = {
              ...
              types: organisation.organisation_type_ids,
        ...
        async saveRelations(){
          ...
          this.currentOrganisations.forEach(relation => {
            if (relation.id){
              let oldRelation = this.initialOrganisations.filter(obj => obj.id = relation.id)[0];
              if (!isEqual(oldRelation, relation)){
                console.log("RELATION NEEDS CHANGE " + relation.id)
              }
            }
            else {
              console.log("RELATION NEEDS CREATION")
            }
          });
        }
      }
