<template>
  <v-container fluid>
    <v-row no-gutters>
      ...
      >
        <v-icon size="small" class="mr-3"> fas fa-plus-circle </v-icon> Create
    ...
    <v-row no-gutters class="mt-2">
      ...
      <v-chip
        ...
        variant="flat"
      >
        <a :href="email" class="text-white">
          <v-icon size="small" class="mr-3"> fas fa-envelope </v-icon> Request
          new species (email)
        </a>
      </v-chip>
    </v-row>
            ...
            <v-col cols="12">
              <v-card width="700px">
                      ...
                      <v-row no-gutters>
                        <v-col cols="12" class="mb-5">
                          ...
                          >
                            ...
                            >Add term to creation list
                          </v-chip>
                        </v-col>
                        <v-col v-if="newTags.length === 0" cols="12">
                          No new tags to create
                        </v-col>
                        <v-col v-else cols="12">
                          Tags to create:
                          <v-chip-group>
                            <v-chip v-for="tag in newTags" :key="'tag_' + tag">
                              {{ tag }}
                              <v-icon
                                class="ml-1"
                                size="small"
                ...
                <v-card-actions>
                  ...
                  <v-btn class="bg-error" @click="showOverlay = false">
                    Cancel
                  </v-btn>
                </v-card-actions>
              </v-card>
    ...
    addTerm() {
      ...
      if (this.newTerm) {
        ...
        if (newTags.indexOf(this.newTerm) === -1) {
          let found = this.allTags.filter(
            (obj) => obj.label.toLowerCase() === this.newTerm,
          );
          if (found.length > 0) {
