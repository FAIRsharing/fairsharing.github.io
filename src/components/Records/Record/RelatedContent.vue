  ...
  methods: {
    ...
    prepareTabsData() {
      ...
      if (_module.currentRecord["fairsharingRecord"].registry === "Policy") {
        _module.tabsData.tabs["related_collections"] = {
          registry: ["Collection"],
          data: [],
          count: 0,
        };
      ...
      // ) {
      //As per the discussion with the team, we are displaying the FAIRassist components for all the record types. https://github.com/FAIRsharing/fairsharing.github.io/pull/2877#issuecomment-5811643400
        ...
        Object.keys(_module.tabsData.tabs).forEach((tabName) => {
          _module.tabsData.tabs[tabName].data = _module
            .prepareAssociations(
              ...
              _module.currentRecord["fairsharingRecord"]
                .reverseRecordAssociations,
            )
            .filter((item) =>
              _module.tabsData.tabs[tabName].registry.includes(item.registry),
            );
          ...
          _module.tabsData.tabs[tabName].data = _module.tabsData.tabs[
            tabName
          ].data.filter(
            (thing, index, self) =>
    ...
    nameAbbrFilter(itemTitle, queryText, item) {
