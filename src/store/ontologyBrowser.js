...
export const actions = {
  async fetchTerms({ commit }) {
    ...
    const { flattenedTree, sunburst } = processTree(
      response["browseSubjects"].data
    );
  ...
  openTerms({ commit }, tabs) {
    commit("setOpenedTerms", tabs);
  },
  leavePage({ commit }) {
    commit("resetTree");
...
export const mutations = {
  ...
  setFlattenedTree(state, tree) {
    state.flattenedTree = tree;
  },
  resetFlattenedTree(state) {
    state.flattenedTree = [];
  },
  ...
  resetPagination(state) {
    state.pagination = { page: 1, perPage: 50 };
  },
  addNodeToSunburst(state, nodeList) {
    state.sunburstData = state.sunburstData.concat(nodeList);
  },
  resetSunburstData(state) {
    state.sunburstData = [SUNBURST_ENTRY];
  },
  setActiveTerms(state, terms) {
    state.activeTerms = terms;
  },
  resetActiveTerms(state) {
    state.activeTerms = [];
  },
  setSelectedTerm(state, term) {
    state.selectedTerm = term;
  },
  resetSelectedTerm(state) {
    state.selectedTerm = null;
  },
  setTotalPages(state, totalPages) {
    state.totalPages = totalPages;
  },
  setOpenedTerms(state, open) {
    state.openedTerms = open;
  },
};
