  ...
  computed: {
    ...
    filterList() {
      ...
      if (
        ...
        let foundItem = output[
          Object.keys(_module.tabsData.tabs)[_module.tabsData.selectedTab]
        ].data.find((item) => item.name === _module.selectedValues);
        if (foundItem) {
          output[Object.keys(output)[_module.tabsData.selectedTab]].data = [];
          output[Object.keys(output)[_module.tabsData.selectedTab]].data.push(
            foundItem,
          );
        }
        return output;
      ...
      joinedArrays.forEach((item) => {
        ...
        properties.forEach((prop) => {
          if (Object.prototype.hasOwnProperty.call(item, prop)) {
            object.recordAssocLabel[0] = _module.cleanString(
              item.recordAssocLabel,
            );
            object.recordAssociationLabel = _module.cleanString(
              item.recordAssocLabel,
            );
