<template>
  <main class="pa-15 mb-10">
    <!--  main_title -->
    <h1 class="text-h4 text-xl-h3 mb-2 mb-xl-6">FAIRsharing API</h1>

    <h2 class="text-h5 text-xl-h4 mb-2 mb-xl-6">Introduction</h2>
    <p
      :class="[
    ...
    >
      The new FAIRsharing application has a REST API which offers the
      opportunity to both query and modify data. It's accessible to any user
      with an account on the system for showing basic information and for
      modifying any record you've created or maintain. To query more detailed
      information from it, please email
      <a
    ...
    <h2 class="text-h5 text-xl-h4 mb-2 mb-xl-6">Creating an account</h2>
    <p
      :class="[
        'mb-4 lato-font-medium lato-text-sm',
        { 'lato-text-md': $vuetify.display.xlOnly },
    ...
    >
      You can create an account by going to the
      <a :href="getHostname()" class="underline-effect">FAIRsharing site</a>,
      clicking the "login" button and then choosing "create new account". You
      can also create an account by signing in with ORCID, Twitter or Github,
      but if you do you'll need to reset your password (click on the "Forgot
      your password?" link) before you can use the API. You will in all cases
      need to <a href="/profiles/edit">edit your user profile</a> to add an
      organisation to which you belong.
    </p>

    <h2 class="text-h5 text-xl-h4 mb-2 mb-xl-6">Using the API</h2>
    <p
      :class="[
        'mb-4 lato-font-medium lato-text-sm',
        { 'lato-text-md': $vuetify.display.xlOnly },
    ...
    <h2 class="text-h5 text-xl-h4 mb-2 mb-xl-6">Logging in</h2>
    <p
      :class="[
        'mb-4 lato-font-medium lato-text-sm',
        { 'lato-text-md': $vuetify.display.xlOnly },
      ...
      <pre>
{
   ...
   "message": "Authentication successful"
    ...
    >
      In addition to the general information about your account there's a "jwt"
      field. This is a JSON web token which is valid until the expiry time
      ...
      <pre>
{
   ...
   "Authorization": "Bearer your_jwt_goes_here"
    ...
    >
      Here are some examples of both logging in and querying a record. Each of
      these queries is for a single FAIRsharing Record with ID of 1.
    </p>
    <p
      :class="[
...
data&lt;-query_con$data
    ...
    >
      We expect that most users will wish to use the API for querying the data
      we hold. However, some who maintain records may like to keep them updated
      in an automated way and so could write scripts which would do this. Some
      may wish to create records, for example if they would like to have their
      data replicated in FAIRsharing; pushing data to FAIRsharing would allow
      such collaborators to update our records immediately to ensure that they
      always reflect the current information in their own portal.
    </p>
    <h3 class="text-h6 text-xl-h5 mt-4 mb-2">Basic parameters</h3>
    <p
      :class="[
        'mb-4 lato-font-medium lato-text-sm',
        { 'lato-text-md': $vuetify.display.xlOnly },
    ...
    >
      To create or modify records you will need to post a FAIRsharing record
      object. For creation, all the terms necessary must be included, but to
      modify a record you need only include the changed terms. Please note,
      though, that the "metadata" field must always be included in its entirety.
      For example, if you changed only the record's name in its metadata then
      ...
      <pre>
{
    ...
    >
      The metadata section contains information about the record itself, such as
      its name, contact details for resource maintainers etc. The full
      specifications for the metadata field, in the form of the JSON schemas
      used to validate it, can be found the following URLs:
    </p>
    ...
    <ul>
      <li>The schema for all FAIRsharing records.</li>
      <li>The relevant registry schema (e.g. the standard schema).</li>
      <li>Any relevant type schema (e.g. the reporting guideline schema).</li>
    </ul>
    ...
    >
      So, the metadata field may contain anything in any of the above three, and
      nothing else.
    </p>
    ...
    >
      The other fields contain references to other objects in our system, such
      as publications or various other tags. To modify these an array of the IDs
      of the relevant objects must be sent to the server. For example, a PUT of
      ...
      <pre>
{
    ...
    >
      In order to determine the IDs of these objects you can POST to the
      following URLs:
    </p>
...
/search/record_types
    ...
    >
      N.B. "taxonomies" is referred to as "species" on the FAIRsharing website's
      pages.
    </p>
    <p
      :class="[
    ...
    >
      Each query has an optional parameter of <mark>q:</mark> if omitted then
      all records will be returned. Otherwise, a search will be performed as
      with <mark>/search/fairsharing_records</mark> (this latter is paginated -
      see earlier for pagination parameters).
    </p>
    ...
    >
      The following fields may be used when performing a POST to
    ...
    >
      "q" is a general search term and will be used to search across all
      records. The other terms will be used to restrict results to those with an
      exact match. Anything marked with "*" takes either "true" or "false".
    </p>
    ...
    >
      Some other objects are not added directly to a FAIRsharing record as
      above; an additional step is necessary to modify them. These are:
    </p>
    <ul>
      <li>Licences.</li>
      <li>Organisations.</li>
      ...
      <pre>
{
  "licence_link":
  {
    ...
    >
      The IDs for licence links can be seen in the licence_link field of your
      record, which you can get according to the initial example given earlier,
      e.g. a GET to <mark>/fairsharing_records/1</mark> (using your record's
      ID). This field will contain both the ID of the licence and the ID of the
      link (only the latter is needed).
    </p>
    <p
      :class="[
        'mb-4 lato-font-medium lato-text-sm',
    ...
    >
      This will add the licence with ID of 1 to the FAIRsharing record with the
      ID of 1. The "relation" field is optional; you may supply any of the
      following values:
    </p>
    <ul>
    ...
    >
      To modify a licence_link, PUT to /licence_links/1, using the ID of the
      licence link.
    </p>
    <p
      :class="[
    ...
    >
      To delete one, use <mark>DELETE /licence_links/1</mark>, again with the ID
      of your link.
    </p>
    <h3 class="text-h6 text-xl-h5 mt-4 mb-2">Organisations and grants:</h3>
    <p
      :class="[
        'mb-4 lato-font-medium lato-text-sm',
        { 'lato-text-md': $vuetify.display.xlOnly },
    ...
    >
      Organisations are linked to FAIRsharing records by means of an
      organisation_link, created similarly to licence_links, above. A grant
      belongs to the relationship between and organisation and FAIRsharing
      ...
      <pre>
{
  "organisation_link":
  {
    ...
    >
      If the relationship is "funds" then you will need to create a grant. This
      is done at the same time as creating the organisation_link:
    </p>
    <p
      :class="[
        'mb-4 lato-font-medium lato-text-sm',
      ...
      <pre>
{
  "organisation": {
    ...
    >
      Organisation types can be found via a GET to
      <mark>/organisation_types.</mark>
    </p>
    <h3 class="text-h6 text-xl-h5 mt-4 mb-2">Related FAIRsharing records:</h3>
    <p
      :class="[
        'mb-4 lato-font-medium lato-text-sm',
        { 'lato-text-md': $vuetify.display.xlOnly },
    ...
    >
      FAIRsharing records can be related to others in a variety of ways. This is
      represented by creating a record_association which describes the linking
      ...
      <pre>
{
  "record_association": {
    ...
    >
      This will create a link from the FAIRsharing record with ID of 1 to that
      with ID of 2, and apply the label with id 3 to this relationship.
    </p>
    <p
      :class="[
        'mb-4 lato-font-medium lato-text-sm',
    ...
    >
      A link can be deleted by means of
      <mark>DELETE /record_associations/1</mark>, supplying the correct ID for
    ...
    >
      The IDs for record associations can be found in the
      <mark>linked_records</mark> field of the data returned from the API for
      your record. Information on labels can be found via a GET to
      <mark>/record_association_labels</mark>.
    </p>
    <p
      :class="[
        'mb-4 lato-font-medium lato-text-sm',
      ...
      <pre>
{
  "record_association_attributes": [
    ...
    >
      The above data will create a new record association between your record
      and the record with ID of 2, assigning the label with ID of 5 to it. Also,
      it will delete the existing record_association with ID of 5.
    </p>
    <p
      :class="[
        'mb-4 lato-font-medium lato-text-sm',
    ...
    >
      Certain relationships are only permitted between certain types or
      registries of records. This is summarised below. If a relationship isn't
      listed then it can be applied between any two records.
    </p>
    ...
    >
      Are you looking for structured data within an individual record's page?
      The FAIRsharing site implements content negotiation, so if you make a
      request for a record, specifying JSON or LD+JSON headers, some basic JSON
    ...
    >
      If you are using this API to update your records you are probably already
      in contact with us. Otherwise, any queries can be directed to
        ...
        relationshipData: {
          ...
          data: [
            ...
            },
            {
              ...
              Restriction: "Any database to any standard",
              Description:
                "This database accepts data that follows a particular standard.",
            },
            {
              id: 2,
              Label: "outputs",
              Restriction: "Any database to any standard",
              Description:
                "This database outputs data that follows a particular standard",
            },
            {
              id: 3,
              Label: "related_to",
              Restriction:
                "Any database to any standard or database, any standard to any standard or database, any policy to any policy",
              Description:
                "This resource is related to another resource in an undefined way",
            },
            {
              id: 4,
              ...
              Description:
                "This database shares a portion of code with another database",
            },
            {
              id: 5,
              ...
              Restriction: "Any database to any other database",
              Description:
                "This database shares a portion of data with another database",
            },
            {
              id: 6,
              Label: "profiles",
            ...
            },
            {
              ...
              Label: "extends",
              Restriction:
                "A database or standard to another of the same registry",
              Description:
