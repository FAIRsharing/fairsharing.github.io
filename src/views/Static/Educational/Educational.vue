<template>
  <main :class="applyCss ? 'pa-15 mb-10' : ''">
    ...
    >
      Across the research disciplines there are thousands of standards and
      several thousand databases, designed to assist the virtuous data cycle,
      from collection to annotation, through preservation and publication to
      subsequent sharing and reuse. As consumers of these standards and
      databases, it is often difficult to know which resources are the most
      relevant for your specific domain and needs. As producers, you want to be
      sure your standard or database is findable by prospective users, and
      recommended in data policies by funders, journals and other organisations.
    </p>
    <p
      :class="[
        'mb-4 lato-font-medium lato-text-sm',
    ...
    >
      With our growing and interlinked content, functionalities and
      endorsements, FAIRsharing is the most comprehensive informative and
      educational resource of standards, databases and data policies and is here
      to help you!
    </p>
    <p
      :class="[
    ...
    >
      The FAIRsharing team works with and for the community to map the landscape
      of community-developed standards and to define the indicators necessary to
      monitor their:
    </p>
    <ul class="hardcoded-ul">
      ...
      <li>
        adoption in data policies by funders, journals and other organisations.
      </li>
    </ul>
    <p
      :class="[
    ...
    >
      Whether you are a researcher, standard/database developer, funder, journal
      editor, librarian or data manager, FAIRsharing can help you understand
      which standards are mature and appropriate to your use case. By mapping
      the relationships between standards and the databases that implement them,
      or the policies that recommend them, FAIRsharing enables you to make an
      informed decision as to which standard or database to use or endorse.
    </p>
    ...
    >
      <v-card class="full-width fill-height" tile>
        <v-img
        ...
        <v-card-title class="justify-center">
          <div v-if="infographicPopup.data.doi" class="d-flex align-center">
            ...
            <v-tooltip location="top">
              ...
              <span v-if="!infographicPopup.data.copyButtonStatus">
                Copy URL
              </span>
              <span v-else> URL copied </span>
            </v-tooltip>
...
export default {
  ...
  async created() {
    ...
    Object.keys(this.education).forEach((item) => {
      Object.keys(this.education[item]).forEach((obj) => {
        let foundHash = this.education[item][obj].find(
          (it) => `#${it.anchorLink}` === this.$route.hash,
        );
        if (foundHash) {
          ...
          const faqNumber = this.$route.hash
            .split("-", 1)
            .toString()
            .substr(4, 2);
          let key = Object.keys(this.education[item])[
            arr.find((item) => item.value === faqNumber).index
          ];
  ...
  methods: {
    ...
    generatePopup() {
      ...
      if (isHash) {
        const hashInfographic = this.infographics.filter(
          (e) => e.hash === hash,
        );
