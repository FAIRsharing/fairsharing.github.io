  ...
  methods: {
    ...
    prepareData() {
      ...
      registries.forEach(function (item) {
        ...
        _module.prepareSubject(
          _module.allDataStats.aggregations[item],
          _module.chartSubjects[item],
          item,
        );
        _module.chartCountries[item] = JSON.parse(JSON.stringify(auxBarChart));
        _module.prepareRecordCountry(
          _module.allDataStats.aggregations[item],
          _module.chartCountries[item],
          item,
        );
        _module.chartSpecies[item] = JSON.parse(JSON.stringify(auxBarChart));
        _module.prepareSpecies(
          _module.allDataStats.aggregations[item],
          _module.chartSpecies[item],
          item,
        );
        if (item !== "policy") {
          _module.chartLicences[item] = JSON.parse(JSON.stringify(auxBarChart));
          _module.prepareLicences(
            _module.allDataStats.aggregations[item],
            _module.chartLicences[item],
            item,
          );
          _module.chartImplements[item] = JSON.parse(
            JSON.stringify(auxBarChart),
          );
          _module.chartJournals[item] = JSON.parse(JSON.stringify(auxBarChart));
          _module.chartHavePublication[item] = JSON.parse(
            JSON.stringify(auxPieChart),
          );
        }
        _module.chartMaintainer[item] = JSON.parse(JSON.stringify(auxPieChart));
        _module.prepareHasMaintainer(
          _module.allDataStats.aggregations[item],
          _module.chartMaintainer[item],
          item,
        );
        ...
        _module.prepareOrganisation(
          _module.allDataStats.top_10_orgs_records[item].funders,
          _module.chartFunders[item],
          item,
          true,
          _module.allDataStats.aggregations[item].countries.doc_count,
        );
        _module.chartNonFunders[item] = JSON.parse(JSON.stringify(auxBarChart));
        _module.prepareOrganisation(
          _module.allDataStats.top_10_orgs_records[item].non_funders,
          _module.chartNonFunders[item],
          item,
          false,
          _module.allDataStats.aggregations[item].countries.doc_count,
        );
      ...
      _module.prepareRecordCountry(
        _module.allDataStats.aggregations.all,
        _module.chartCountriesAll,
        "record",
      );
      _module.prepareImplementAndJournal(
        _module.allDataStats.top_10_stds_recommended_by_pols,
        _module.chartImplements["standard"],
        "standard",
      );
      _module.prepareImplementAndJournal(
        _module.allDataStats.top_10_std_recs_journal,
        _module.chartJournals["standard"],
        "journalStand",
      );
      _module.prepareImplementAndJournal(
        _module.allDataStats.top_10_db_recs_journal,
        _module.chartJournals["database"],
        "journalDB",
      );
      _module.prepareImplementDB(
        _module.allDataStats.dbs_implementing_most_stds,
        _module.chartImplements["database"],
      );
      _module.prepareLinkPie(
        _module.allDataStats.stds_implemented_by_a_db,
        _module.chartLinkPie["standard"],
        "Standards that have been implemented by a database",
      );
      _module.prepareLinkPie(
        _module.allDataStats.dbs_linked_to_stds,
        _module.chartLinkPie["database"],
        "Databases that are linked to one or more standard",
      );
      _module.prepareLinkPie(
        _module.allDataStats.pols_linked_to_std_or_db,
        _module.chartLinkPie["policy"],
        "Policies that are linked to one or more standard or database",
      );
      _module.prepareHasPublication(
        _module.allDataStats.stds_to_pubs,
        _module.chartHavePublication["standard"],
        "standard",
      );
      _module.prepareHasPublication(
        _module.allDataStats.dbs_to_pubs,
        _module.chartHavePublication["database"],
        "database",
      );
    },
    ...
    prepareLicences(data, chartField, text) {
      ...
      let nameMap = {
        ...
        "apache license 2.0": "Apache Licence 2.0",
        "w3c document license": "W3C Document Licence",
        "bsd-3-clause license (modified bsd license) (new bsd license)":
        ...
        "creative commons attribution-sharealike 4.0 international (cc by-sa 4.0)":
          "CC BY-SA 4.0",
        "umls metathesaurus license": "UMLS Metathesaurus Licence",
        "gnu general public license (gpl) 3.0": "GPL 3.0",
        "creative commons attribution-noncommercial-noderivatives 4.0 international (cc by-nc-nd 4.0)":
          "CC BY-ND 4.0",
        "creative commons attribution-noncommercial-sharealike 3.0 unported (cc by-nc-sa 3.0)":
          "CC BY-NC-SA 3.0",
        "ogc document notice": "OGC Document Notice",
        "creative commons attribution 2.0 generic (cc by 2.0)": "CC BY 2.0",
        "library of congress legal information":
          "Library of Congress Legal Information",
        ...
        "creative commons attribution-noncommercial 4.0 international (cc by-nc 4.0)":
          "CC BY-NC 4.0",
        "gnu lesser general public license (lgpl) 2.1": "LPGL 2.1",
        "nih web policies and notices": "NIH Web Policies and Notices",
    ...
    prepareHasPublication(data, chartField, text) {
      chartField.title =
        text[0].toUpperCase() +
        text.substring(1) +
        " records that have publications";
      let nameMap = {
        standards_with_publication: "Yes",
        standards_without_publication: "No",
        databases_with_publication: "Yes",
        databases_without_publication: "No",
      };
      Object.keys(data).forEach((key) => {
        let vectItem = {
          name: nameMap[key],
