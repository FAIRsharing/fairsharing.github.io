...
describe("EditRelationships.vue", function () {
  ...
  it("can watch and react to search and associations changes", async () => {
    ...
    expect(wrapper.vm.availableRecords).toStrictEqual([
      { id: 1, isActive: true },
    ]);
    let pushItem = {
      id: 123,
    ...
    await recordStore.state.sections.relations.data.recordAssociations.push(
      pushItem,
    );
  ...
  it("can open the overlay to add a new association", () => {
    ...
    wrapper.vm.addItem();
    expect(
      wrapper.vm.sections.relations.data.recordAssociations[0],
    ).toStrictEqual({
      new: true,
  ...
  it("can save again", async () => {
    ...
    recordStore.state.sections.relations.initialData.recordAssociations =
      relations;
    wrapper.vm.showOverlay({
      name: "yolo",
      id: 333,
