...
let store = {
  state: {
    users: {
      user: function () {
        return { isLoggedIn: true, is_super_curator: true };
      },
    },
    introspection: {
...
describe("Routes", () => {
  ...
  it("performs hardcoded record redirections correctly", async () => {
    ...
    expect(
      article.redirect({ params: { name: "live_list_standards_in_policies" } }),
    ).toStrictEqual({
      name: "search",
      query: { fairsharingRegistry: "Standard", isRecommended: true, page: 1 },
    });
    expect(
      article.redirect({ params: { name: "live_list_databases_in_policies" } }),
    ).toStrictEqual({
      name: "search",
      query: { fairsharingRegistry: "Database", isRecommended: true, page: 1 },
    });
    expect(
      article.redirect({ params: { name: "live_list_journal_policies" } }),
    ).toStrictEqual({
      name: "search",
      query: { fairsharingRegistry: "Policy", recordType: "journal", page: 1 },
    });
    expect(article.redirect({ params: { name: "dingdong" } })).toStrictEqual({
      path: "/",
    });
