...
describe("CuratorCohorts.vue", function () {
  ...
  it("can be instantiated", () => {
    ...
    expect(wrapper.vm.communityCurationCohorts.data).toStrictEqual(
      realData.data,
    );
    let currentCuratorsList = realData.data.filter((curator) => {
      return (
        curator.year_active.includes(new Date().getFullYear().toString()) &&
        curator.active
      );
    });
  ...
  it("can alumniCurator is true on click", () => {
    ...
    let currentCuratorsList = realData.data.filter((curator) => {
      return (
        curator.year_active.every((el) => el < new Date().getFullYear()) ||
        !curator.active
      );
    });
    expect(wrapper.vm.currentCohort).toStrictEqual(currentCuratorsList);
