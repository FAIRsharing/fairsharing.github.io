...
describe("Statistics.vue", () => {
  ...
  it("can be mounted and data properly created", async () => {
    ...
    expect(wrapper.vm.chartSubjects["policy"].textYAxis).toBe(
      "Number of policies",
    );
    expect(wrapper.vm.chartSubjects["policy"].series[0].name).toBe(
      "Natural Science",
    );
    expect(wrapper.vm.chartSubjects["standard"].series[2].data[0].z).toBe(
      (100 * 4) / 74,
    );
    ...
    expect(wrapper.vm.chartSpecies["policy"].title).toBe(
      "Top 10 species covered by policies",
    );
    ...
    expect(wrapper.vm.chartLicences["database"].title).toBe(
      "Top 10 licenses for database content",
    );
    expect(wrapper.vm.chartLicences["standard"].series[0].name).toBe(
      "CC BY 3.0",
    );
    expect(wrapper.vm.chartLicences["database"].series[1].data[0].y).toBe(15);

    expect(wrapper.vm.chartMaintainer["policy"].title).toBe(
      "Policy records that have maintainers",
    );
    ...
    expect(wrapper.vm.chartMaintainer["policy"].data[1].url).toBe(
      "/search?fairsharingRegistry=policy&isMaintained=false",
    );
    ...
    expect(wrapper.vm.chartFunders["standard"].series[0].name).toBe(
      "org1, Germany",
    );
    ...
    expect(wrapper.vm.chartNonFunders["standard"].title).toBe(
      "Top 10 organisations (excluding funders) of standards",
    );
